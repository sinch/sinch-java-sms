...
public class ApiConnectionIT {
  ...
  public void canHandleBatchCreateWithError() throws Exception {
    ...
    ApiError apiError =
        ApiError.of("syntax_constraint_violation", "The syntax constraint was violated");
  ...
  public void canHandle500WhenFetchingBatch() throws Exception {
    ...
    try {
      ...
      assertThat(
          response.getEntity().getContentType().getValue(), is(ContentType.TEXT_PLAIN.toString()));
  ...
  /**
   * Verifies that the default HTTP client actually can handle multiple simultaneous requests.
   *
  ...
  public void canFetchDeliveryReportSync() throws Exception {
    ...
    BatchDeliveryReportParams filter =
        SinchSMSApi.batchDeliveryReportParams()
            .summaryReport()
            .addStatus(DeliveryStatus.ABORTED, DeliveryStatus.DELIVERED)
            .addCode(200, 300)
            .build();
  ...
  public void canCreateBatchDryRunSync() throws Exception {
    ...
    final MtBatchDryRunResult expected =
        MtBatchDryRunResult.builder().numberOfRecipients(20).numberOfMessages(200).build();
  ...
  public void canListGroupsWithEmpty() throws Exception {
    ...
    final Page<GroupResult> expected =
        PagedGroupResult.builder().page(0).size(0).totalSize(0).build();
  ...
  public void canListGroupsWithTwoPages() throws Exception {
    String spid = TestUtils.freshServicePlanId();
    GroupFilter filter = SinchSMSApi.groupFilter().addTag("tag1", "таг2").build();

    // Prepare first page.
    String path1 = "/v1/" + spid + "/groups?page=0&tags=tag1%2C%D1%82%D0%B0%D0%B32";

    final Page<GroupResult> expected1 =
        PagedGroupResult.builder().page(0).size(0).totalSize(2).build();
    ...
    final Page<GroupResult> expected2 =
        PagedGroupResult.builder().page(1).size(0).totalSize(2).build();
  ...
  public void canListInboundsWithTwoPages() throws Exception {
    String spid = TestUtils.freshServicePlanId();
    InboundsFilter filter = SinchSMSApi.inboundsFilter().addRecipient("10101").build();
  ...
  private void verifyDeleteRequest(String path) {
    wm.verify(
        deleteRequestedFor(urlEqualTo(path))
            .withHeader("Accept", equalTo("application/json; charset=UTF-8"))
            .withHeader("Authorization", equalTo("Bearer tok")));
  }
   ...
   * @throws JsonProcessingException if the given request object could not be serialized
   */
