...
/**
 * The result of a message batch dry-run. The XMS API allows users to simulate sending batches and
 * this result class contains information that allow you to analyze various aspects of a batch run
 * before it is actually performed.
 *
 * <p>It is, for example possible to see how many actual SMS messages will be sent as part of the
 * batch.
 */
@Value.Immutable
@ValueStylePackage
...
public abstract class MtBatchDryRunResult {
  ...
  public abstract static class PerRecipient {
    ...
    /**
     * The message body sent to this recipient. This includes template expansion.
     *
    ...
    /**
     * The message encoding that will be applied for this recipient. This is one of "text" or
     * "unicode".
     *
  ...
  /**
   * Information on a per-recipient level. When requesting a dry-run this will be populated in the
   * response only if per-recipient information is also requested and only if the batch is textual.
   *
