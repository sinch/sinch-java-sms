...
public abstract class BatchDeliveryReportParams {
  ...
  public static enum ReportType {
    /**
     * Indicates a summary batch delivery report. The summary delivery report does not include the
     * per-recipient result but rather aggregated statistics about the deliveries.
     */
    SUMMARY,

    /**
     * Indicates a full batch delivery report. This includes per-recipient delivery results. For
     * batches with many destinations such reports may be very large.
     */
    FULL
  }
  ...
  List<NameValuePair> toQueryParams() {
    ...
    if (reportType() != null) {
      params.add(new BasicNameValuePair("type", reportType().name().toLowerCase(Locale.US)));
    }
    ...
    if (!statuses().isEmpty()) {
      ...
      params.add(new BasicNameValuePair("status", Utils.join(",", statusStrings)));
    }

    if (!codes().isEmpty()) {
      ArrayList<String> codeStrings = new ArrayList<String>(codes().size());

      for (Integer code : codes()) {
        codeStrings.add(code.toString());
      }

      params.add(new BasicNameValuePair("code", Utils.join(",", codeStrings)));
    }
